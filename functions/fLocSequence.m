classdef fLocSequence
    ...
    properties (Constant)
        stim_conds = {'Characters-L', 'Faces-L', 'Scrambled-L', ...
                       ...
                       'Characters-R', 'Faces-R', 'Scrambled-R'};
    ...
    methods
        ...
        function seq = make_runs(seq)
            ecc_by_cond = [0 seq.eccentricity_vals(1) seq.eccentricity_vals(1) seq.eccentricity_vals(1), ...
                seq.eccentricity_vals(2) seq.eccentricity_vals(2) seq.eccentricity_vals(2), ...
                seq.eccentricity_vals(3) seq.eccentricity_vals(3) seq.eccentricity_vals(3)]; % initial zero for baseline condition
            ...
            ecc_mat = zeros(seq.stim_per_block, seq.num_conds ^ 2 + 2, seq.num_runs);
            for rr = 1:seq.num_runs
                cat_list = ['baseline' seq.run_sets(rr, :)];
                cat_seq = cat_list(block_conds(:, rr) + 1);
                ecc_seq = ecc_by_cond(block_conds(:,rr) + 1);
