classdef fLocSession
    ...
    properties
        ...
        eccentricity_values % e.g. a list of 3, or a single int, e.g. [-6,0,6] or 6
        screen_id
        viewdist_mm
        screenwidth_mm
        screenwidth_pix
    ...
    methods
        ...
        % class constructor
        function session = fLocSession(name, trigger, stim_set, num_runs, task_num, atscanner, screen_id, eccentricity_values)
            ...
            if nargin < 8
                session.eccentricity_values = [-6, 0, 6];
            else
                session.eccentricity_values = eccentricity_values;
            end
        ...
        function session = load_seqs(session)
            ...
            if ~exist(fpath, 'file')
                seq = fLocSequence(session.stim_set, session.num_runs, session.task_num, session.eccentricity_values);
                seq = make_runs(seq);
...
%             session.input = [];
        ...
        function session = run_exp(session, run_num)
            ...
            start_time = GetSecs;
            for ii = 1:length(stim_names)
                % determine location of this trial
                shift_x = deg2pix(session.sequence.ecc_trials(ii,run_num), ...
                    session.viewdist_mm, session.screenwidth_mm, session.screenwidth_pix); % compute stimulus eccentricity in pix
